...
| T2 | Attacker tampers with `scope` to elevate privileges | **Scope is a privilege boundary, and it only ever narrows.** A requested scope is validated against the operator-enabled capabilities at `/authorize`, re-validated at `/consent` (so tampering with the hidden form field is caught), and resolved again when the code is redeemed. `scopeToCapabilities` grants a scope token only if *every* operation it names is enabled on the server — no partial widening — and a single unknown, malformed or disabled token rejects the whole grant with `invalid_scope`. Tampering can therefore only ask for less, or for nothing. An omitted scope is the RFC 6749 §3.3 pre-defined default: the full operator grant, stated as such on the consent screen. | T4.4 |
...
| # | Threat | Mitigation | Task |
|---|--------|------------|------|
| E1 | OAuth token used to access a collection capability beyond what the operator enabled | `MCPAccessSettings` is built at validation time by **intersecting** the token's stored grant with the capabilities enabled on the MCP server *at that moment* (`buildFullCapabilities(mcpPluginOptions)`), never from the client's registration or self-asserted claims. The live config is therefore a hard ceiling a token can never exceed, and disabling a collection (or a single operation on it) narrows every existing token on the next request rather than when the last token issued before the change expires. A scoped grant is additionally capped by the narrower set it was issued with; a no-scope grant means “whatever is enabled now” and tracks the config in both directions, which is the standing instruction the user consented to. Tokens also record the grant they were issued with, so a row is auditable on its own — up to 0.4.0 an omitted scope stored `{}`, which read as “no permissions” while requests ran with full access (#75). | T3.5, T4.5 |
